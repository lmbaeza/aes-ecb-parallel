...
vector<vector<uint8>> text_to_hex(const string &text) {
    ...
    vector<vector<uint8>> text_hex = vector<vector<uint8>>(blocks, vector<uint8>(blocks_size, 0));
    int idx_block = 0;
    int block = 0;
    for(int i = 0; i < n; ++i) {
        text_hex[block][idx_block] = uint8(text[i]);
        ++idx_block;
        if(idx_block == 16) {
            idx_block = 0;
            block++;
...
vector<uint8> key_to_hex(const string &key) {
    int n = (int) key.size();
    assert(0 <= n && n <= 32);
    vector<uint8> key_hex = vector<uint8>(32, 0);
    for(int i = 0; i < n; ++i) {
...
int main() {
    ...
    vector<vector<uint8>> Encript;
    vector<vector<uint8>> Decript;

    for(int i = 0; i < (int) text_hex.size(); ++i) {
        uint len = 0;
        vector<uint8> cipher = aes.EncryptECB(text_hex[i], BLOCK_BYTES_LENGTH, key_hex, len);
        vector<uint8> dec = aes.DecryptECB(cipher, BLOCK_BYTES_LENGTH, key_hex);

        Encript.push_back(cipher);
    ...
    cout << "\nDecript Text: ";
    for(int i = 0; i < (int) Decript.size(); ++i) {
        for(int j = 0; j < (int) Decript[i].size(); ++j) {
            cout << (char) Decript[i][j];
        }
    }
    cout << endl;
    // Out:
